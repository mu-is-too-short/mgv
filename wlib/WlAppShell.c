 ...
 */
#include <wlib/rcs.h>
MU_ID("$Mu: mgv/wlib/WlAppShell.c 1.1 2001/03/09 04:16:27 $")
...
static	int	cls;
	...
	name = wl_lstrdup((char *)from->addr);
	cls  = DEFAULT;
...
valid_vid(Widget w, XVisualInfo *vi)
{
	...
	if((vis = XGetVisualInfo(wl_display(w), VisualIDMask, &tmp, &n)) == NULL)
		return False;
...
/*
 * The LessTif folks have wisely added editres support to their VendorShell
 * implementation so we don't have to bother with it.
 */
