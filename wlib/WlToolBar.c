 ...
 */
#include <wlib/rcs.h>
MU_ID("$Mu: mgv/wlib/WlToolBar.c 1.2 2001/03/14 15:01:04 $")
...
tip_post(XtPointer closure, XtIntervalId *unused)
{
	...
	if(!XQueryPointer(wl_display(tb), XtWindow(tb), &root, &win,
							&rx, &ry, &x, &y, &mask)
	|| win == None)
		return;

	w = XtWindowToWidget(wl_display(tb), win);
	if(w == NULL
	|| XtParent(w) != tb
	|| tbc_tooltip(w) == NULL)
		return;
	...
	if(!XGetGeometry(wl_display(tb), win, &root, &x, &y,
...
static void
sizer(Widget w, XtWidgetGeometry *g)
{
	int    i, kids;
	Widget kid;
...
get_icons_int(Widget w, char *name, Pixel fg, Pixel bg, Pixmap *p, Pixmap *ip)
{
	...
	*p = *ip = None;
	...
	if(tb_use_color_icons(w)) {
		*p  = wlp_xpm_from_data(w, wl_display(w), bg, e->xpm);
		*ip = wlp_xpm_from_data(w, wl_display(w), bg, e->ixpm);
	}

	if(*p == None)
		*p  = wlp_xbm_from_data(w, wl_display(w), fg, bg,
					(char *)e->bits, e->width, e->height);
	if(*ip == None)
		*ip = wlp_xbm_from_data(w, wl_display(w), fg, bg,
...
{
	int  len     = strlen(name);
	char *fname  = wl_malloc(len + 5);
	char *ifname = wl_malloc(len + 5 + 2);
	...
	if(tb_use_color_icons(w)) {
		strcpy(ext,  "xpm");
		*p = wlp_xpm_from_file(w, wl_display(w), bg, fname);

		strcpy(iext, "xpm");
		*ip = wlp_xpm_from_file(w, wl_display(w), bg, ifname);
	}

	if(*p == None) {
		strcpy(ext, "xbm");
		*p = wlp_xbm_from_file(w, wl_display(w), fname);
	}
	if(*ip == None) {
		strcpy(iext, "xbm");
		*ip = wlp_xbm_from_file(w, wl_display(w), ifname);
	}
...
get_unknowns(Widget w, Pixel fg, Pixel bg, Pixmap *p, Pixmap *ip)
{
	*p = *ip = None;
	...
	if(tb_use_color_icons(w)) {
		*p  = wlp_xpm_from_data(w, wl_display(w), bg, tb_unknown_xpm);
		*ip = wlp_xpm_from_data(w, wl_display(w), bg, tb_iunknown_xpm);
	}
...
destroy(Widget w)
{
	...
	wl_free_pixmap(wl_display(w), tb_unknown(w));
	wl_free_pixmap(wl_display(w), tb_iunknown(w));
}
...
#	define CANT_LOAD "Could not load icon %s%s for %s"
	...
	for(i = 0; i < comp_num_children(w); ++i) {
		...
		if(p == None) {
			String buf = wl_malloc(sizeof(CANT_LOAD)
...
set_values(Widget cur, Widget req, Widget neww, ArgList a, Cardinal *n)
{
	...
	if(tb_spacing(cur)       != tb_spacing(neww)
...
constraint_destroy(Widget w)
{
	...
	if(p  != None && p  != tb_unknown(tb))
		XFreePixmap(wl_display(tb), p);
	if(ip != None && ip != tb_iunknown(tb))
		XFreePixmap(wl_display(tb), ip);
}
