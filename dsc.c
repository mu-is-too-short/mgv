 ...
 */
#include <wlib/rcs.h>
MU_ID("$Mu: mgv/dsc.c 1.27 1999/09/22 17:34:48 $")
...
dsc_scan(FILE *file)
{
	...
	if (line[0] != '%' ||
		...
		dsc->media[dsc->n_media].name = wl_stolower(cp);
	...
	while (readline(line, sizeof line, file, &position, &line_len) &&
		...
		cp = wl_stolower(gettext(line+length("%%PageMedia:"), NULL));
...
continuepage:
	while (readline(line, sizeof line, file, &position, &line_len) &&
		...
		free(cp);
		       ...
		       iscomment(line+2, "PaperSize:")) {
		cp = wl_stolower(gettext(line+length("%%PaperSize:"), NULL));
		for (dmp = dsc->media, i=0; i<dsc->n_media; i++, dmp++) {
		    if (strcmp(cp, dmp->name) == 0) {
			dsc->pages[dsc->n_pages].media = dmp;
			break;
